...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (avatarFile && avatarFile.buffer) {
        try {
            avatar = await uploadBufferToCloudinary(avatarFile.buffer);
        } catch (error) {
    ...
    if (coverImageFile && coverImageFile.buffer) {
        try {
            coverImage = await uploadBufferToCloudinary(coverImageFile.buffer);
        } catch (error) {
            console.error("Cloudinary cover image upload error:", error);
            // Not throwing error, coverImage is optional
        }
    }
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    if (!username && !email) { // Alternative way (!(username || email))
